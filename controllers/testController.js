...
export const createTest = async (req, res) => {
  try {
    const { title, youtubeLink, isPublic } = req.body;
    const pdfUrl = req.file ? `/uploads/${req.file.filename}` : null;

    // Generate a unique access code for private tests
    const accessCode = !isPublic ? crypto.randomBytes(3).toString("hex") : null;
